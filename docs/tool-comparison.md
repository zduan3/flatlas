...
本文记录 File Atlas（`flatlas`）与已调研的同类工具的功能边界。它是产品范围记录，不要求复刻查重工具的命令行参数或交互；基础查询命令 `ls` / `du` / `df` 另以 GNU Coreutils 作为用户心智模型，详见 [`ls` / `du` / `df` 查询命令设计](query-cli-design.md)。
...
- 阶段 2：配置化过滤、当前状态校验、dirty state、历史策略和故障测试。
