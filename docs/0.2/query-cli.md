...
> 状态：0.2 规划。除明确标记为已实现的内容外，不代表当前 CLI 已支持。

## 状态与目标

本文定义 File Atlas 0.2 的查询体验与兼容性规划。0.1 已完成并冻结；当前已交付行为以 [0.1 实现状态](../0.1/implementation.md) 为准。本文中的“目标默认行为”和参数均为后续规划，不代表当前 CLI 已支持。
