...
本文记录不纳入 Python MVP 的后续能力。当前基线是：`scan` 只采集 metadata；`dupes PATH` 只为 PATH 子树内的同大小普通文件计算 hash，使用 size → quick → full 流水线、前后 `fstat` 校验和保守缓存复用。已完成的 hash 分批提交，因此中断 `dupes` 不会丢失此前已提交的缓存；这不等同于完整的 hash 运行审计或可恢复任务。
