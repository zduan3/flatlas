...
flatlas largest D:\Archive\2026 -n 50
...
flatlas largest [PATH] [-n N|--limit N] [--format table|json|csv]
...
目录的 `part` / `gone` 状态可以附带最后已知聚合，但该值可能不完整或过期；未知必须显示为 `-` / `null`，不能伪装为 0。0.2 的查询演进规划见 [查询命令规划](docs/0.2/query-cli.md)。
